## this transaction will go through if at least 1 of the conditions are met, price met, time limit met, Pub key hash there.
## we will also be using the public key hash to keep the public keyh from being published
## time constrainst are in POSIX time  -- universal UNIX time in milliseconds from jan 1 midnight  GMT 1971
...
saveDatum :: IO ()
saveDatum  = writeDataToFile "./testnet/datum.json" (Conditions "a2a15a1901d0229101bcb31629629210ce8d2ccf058d05afea33e273" 1686837045000 50) ## pkh, slot# and price ADA
...
utxoin="eb8e33b9cc85a8d73a8ad5afafc496cce8660aba4b64c6a7ac7eeb5ea973aff7#0"
...
ownerPKH=$(cat ../../WalletMine/4paytment2.pkh)
...
cardano-cli transaction build \
  ...
  --required-signer-hash $ownerPKH \        ## will separate public key hashs for owner and signer if they are not the same
  --tx-in-collateral $collateral \
  --tx-out $address+$output \
  ## --tx-out $address+$output                   --adding this will send some back to the conditionator.addr with the same datum as before
  ## --tx-out-datum-hash-file datum.json
  --change-address $nami \
  --protocol-params-file protocol.params \
...
cardano-cli transaction sign \
    ...
    --signing-key-file ../../WalletMine/5payment3.skey \
    --signing-key-file ../../WalletMine/5payment3.skey \     -- would need a second signature id colleteral and the owner were different
    $PREVIEW \
