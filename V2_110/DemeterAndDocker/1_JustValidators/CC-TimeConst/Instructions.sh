## CommonConditions.hs
## lock and redeem with time constraint


### Query tip
cardano-cli query tip --testnet-magic 2
### will give block epoch era slot#...

## https://www.epochconverter.com/#:~:text=What%20is%20epoch%20time%3F,01T00%3A00%3A00Z).
## time constraints work in POSIX Time (unix Epoch Time GMT)
## current time can be got from the internet EpockConverter UnixTimeStamp
## to work out a time in the future you need to add the time in milliseconds
## 1 hour = 1 * 60 * 60 * 1000  = 3,600,000
## 


## the signer is the 1ent107.pkh
## 832bb3d5216092580bb596b6fd35ae845c63adececd7958466ec38a5


## UTXO from 4stake2.addr
## 4payment2.skey


## the minimum value is 50 ADA


# bash $ cabal repl
# $ import Plutus.V1.Ledger.Interval           # will give some errors in this examples as Prelude not loaded
# $ :t to
to :: a -> Interval a
# $ :t from
from :: a -> Interval a
# $ :t contains
contains :: PlutusTx.Ord.Ord a => Interval a -> Interval a -> Bool
# $ :t overlaps
overlaps :: (PlutusTx.Enum.Enum a, PlutusTx.Ord.Ord a) => Interval a -> Interval a -> Bool

# $ z = to 10
# $ z
Interval {ivFrom = LowerBound NegInf True, ivTo = UpperBound (Finite 10) True}
...
cardano-cli transaction build \
  ...
  --tx-in-redeemer-file redeemOwner.json \          ## was redeemTime.json
  --required-signer-hash $collateralPKH \
  --required-signer-hash $signerPKH \                ## added for redeemOwner.json
...
cardano-cli transaction sign \
    ...
    --signing-key-file ../../WalletMine/1ent107.skey \         ## added for redeemOwner.json
    $PREVIEW \
